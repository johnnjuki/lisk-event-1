...
describe("MockNFTMarketplace", function () {
  async function deployMockNFTMarketplaceFixture() {
    ...
    console.log(marketplace, owner, otherAccount);
  ...
  describe("Deployment", function () {
    it("Should set the right NFT price", async function () {
      const { marketplace } = await loadFixture(
        deployMockNFTMarketplaceFixture
      );
      expect(await marketplace.getPrice()).to.equal(ethers.parseEther("0.1"));

      it("Should ensure a token is availabe", async function () {
        const { marketplace } = await loadFixture(
          deployMockNFTMarketplaceFixture
        );
        expect(await marketplace.isAvailable(1)).to.equal(false);
      });
    });
    ...
    describe("Purchase", function () {
      ...
      it("Should not allow purchasing an NFT if payment is incorrect", async function () {
        const { marketplace } = await loadFixture(
          deployMockNFTMarketplaceFixture
        );

        await expect(
          marketplace.purchase(1, { value: ethers.parseEther("0.2") })
